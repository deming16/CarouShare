...
router.get('/bid', async (req, res, next) => {
    try {
        if (req.isAuthenticated()) {
            const query =
                'select biid, title, status, item_iid from Bids B inner join Listings L on (B.listing_lid = L.lid) where B.bidder_uid = $1 and L.status = $2';
...
router.post('/loan/end/:listingId', async (req, res, next) => {
    ...
    try {
        if (req.isAuthenticated()) {
            ...
            // get loan bid id
            let query =
                'select biid, listing_lid, bidder_uid, item_iid, title from Bids inner join Loans on (bid_biid = biid) inner join Listings on (listing_lid = lid) where listing_lid = $1';
